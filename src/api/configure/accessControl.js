...
const accessControl = {
  ...
  verify({ accessType, table, user }) {
    const accessValues = get(
      this.grantList,
      `${user.role}.${table}.${accessType}`,
      false
    );
    if (accessValues) {
      accessValues.filter = JSON.parse(
        JSON.stringify(accessValues.filter || {}).replace(
          "/=currentUser/g",
          user._id
        )
      );
      if (accessType === "update_any") {
        accessValues.filter_access = get(
          this.grantList,
          `${user.role}.${table}.read_any`,
          false
        );
      } else if (accessType === "update_own") {
        accessValues.filter_access = get(
          this.grantList,
          `${user.role}.${table}.read_own`,
          false
        );
      }
      return {
  ...
  each(Object.keys(schema), field => {
    if (typeof schema[field] === "string") {
      ...
      if (!gotReferenceField) {
        ...
        const tablePermissions = get(
          grantList,
          `${role}.${gotReferenceField}.${access}`,
          {}
        );
        const refFields = permissions({
          allSchemaFields,
          table: gotReferenceField,
...
const permissions = ({
  ...
  if (allowedFields[0] === "*") {
    ...
    console.log("table, allowedFields", table, allowedFields);
    console.log("allSchemaFields[table]", allSchemaFields[table]);
    each(allowedFields, field => {
      if (allSchemaFields[table][field]) {
        ...
        throw new Error(
          `${field} not found in schema definition of table ${table}`
        );
      }
    });
  }
...
const checkTypes = (definition, field, fields) => {
  if (
    planeDataTypes.includes(definition.type) ||
    planeDataTypes.includes(definition)
  ) {
