...
const { each, get, omitBy, isNil, isEmpty } = require("lodash");
const APIError = require("../utils/APIError");

const MongoOperators = ["$set", "$inc", "$push", "$min", "$max", "$addToSet"];
...
const mongooseModel = {
  addCollection(collectionName, collectionFields, hooks, methods) {
    const collectionSchema = new mongoose.Schema(collectionFields, {
      timestamps: true
    });
    /*
    ...
    */
    ...
    collectionSchema.statics = {
      ...
      async findAndGenerateToken(options) {
        ...
        if (user.deleted) {
          ...
          err1.message = "User Deleted";
          throw new APIError(err1);
        }
        if (user.status === false) {
          ...
          err1.message = "User Blocked";
          throw new APIError(err1);
        }
        if (password) {
          if (user && (await user.passwordMatches(password))) {
            console.log("User details inside mongoose modal in login", user);
            if (user.role === "Patient") {
              if (!user.phoneVerified) {
                console.log("Inside emailverification", user);
                err.message = {
    ...
    collection[collectionName] = mongoose.model(
      collectionName,
      collectionSchema
    );
  },
  ...
  setterMakerAndValidator({ access, reqParams, user }) {
    ...
    each(Object.keys(reqParams.setter), op => {
      if (MongoOperators.indexOf(op) > -1) {
        ...
        if (access[op]) {
          ...
          console.log(
            "access[op] ",
            access[op].allowedFields[0].trim() === "*"
          );
        ...
        throw new APIError({
          message: `unkonwn top level operator [${op}] in update setter`,
          status: httpStatus.FORBIDDEN
        });
      }
    });
    ...
    return {
      filter,
      setter: reqParams.setter
    };
  },
  insertMakerAndValidator({ access, data }) {
    const setterFields = {};
...
const populate = (
  ...
  if (data) {
    ...
    each(Object.keys(data.fields), field => {
      if (
        !nestedParamsFields ||
        nestedParamsFields === 1 ||
        nestedParamsFields[field]
      ) {
        const newNestedParamsFields =
          typeof nestedParamsFields === "object" ? nestedParamsFields : {};
        stack.path = path;
        stack.model = data.ref;
        if (
          typeof data.fields[field] === "object" &&
          typeof nestedParamsFields[field] === "object"
        ) {
          console.log("fields[field].ref;", data.ref);
          const newStack = populate(
            data.fields[field],
            populateStack,
            level + 1,
            {},
            field,
            newNestedParamsFields[field]
          );
          if (
            newStack &&
            newStack.select &&
            Object.keys(newStack.select).length > 0
          ) {
  ...
  each(Object.keys(reqParams.fields), field => {
    if (access.fields[`${appendKey}${field}`]) {
      if (access.fields[`${appendKey}${field}`] === 1) {
        topLevelFields = topLevelFields
          ? `${topLevelFields} ${appendKey}${field}`
          : `${appendKey}${field}`;
      } else if (
        typeof access.fields[`${appendKey}${field}`] === "object" &&
        access.fields[`${appendKey}${field}`].ref
      ) {
        topLevelFields = topLevelFields
          ? `${topLevelFields} ${appendKey}${field}`
          : `${appendKey}${field}`;
        const stack = populate(
          access.fields[`${appendKey}${field}`],
          populateStack,
          0,
          {},
          field,
          reqParams.fields[`${appendKey}${field}`]
        );
        populateStack.push(stack);
      }
    } else {
      const childField = get(
        parentRequestParams.fields,
        `${appendKey}${field}`,
        false
      );
