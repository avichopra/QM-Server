 ...
 * @public
 */
exports.get = async ({ query, body, params, user, access }) => {
	...
	try {
		...
		const data = mongooseModel.queryMakerAndValidator({
			access,
			user,
			reqParams: { ...query, ...params, ...body }
		});
 ...
 * @public
 */
exports.create = async ({ query, body, params, user, access }) => {
	try {
		const Collection = mongooseModel.getCollection(params.collection);
 ...
 * @public
 */
exports.update = async ({ query, body, params, user, access }) => {
	try {
		const Collection = mongooseModel.getCollection(params.collection);
		const data = mongooseModel.setterMakerAndValidator({
			access,
			user,
			reqParams: { ...query, ...params, ...body }
		});
		console.log('setter...', data);
		const users = await Collection.update(data.filter, data.setter, {
			multi: true
		});
 ...
 * @public
 */
exports.list = async ({ query, body, params, user, access }) => {
	console.log('query, body, params, user,', query, body, params, user, access);
	try {
		const Collection = mongooseModel.getCollection(params.collection);
		const data = mongooseModel.queryMakerAndValidator({
 ...
 * @public
 */
exports.count = async ({ query, body, params, user, access }) => {
	console.log('query, body, params, user,', query, body, params, user, access);
	try {
		const Collection = mongooseModel.getCollection(params.collection);
		const data = mongooseModel.queryMakerAndValidator({
 ...
 * @public
 */
exports.remove = async ({ query, body, params, user, access }) => {
	console.log('query, body, params, user,', query, body, params, user, access);
	try {
		const Collection = mongooseModel.getCollection(params.collection);
		const data = mongooseModel.queryMakerAndValidator({
				...
				if (isValidRequest) {
					if (fileUpload.type == 'local') {
						try {
							...
							return download(`${appRoot}/uploads/${bucket}/${filename}`)
								.then((file) => {
									resp.send(file);
									return resolve(file);
								})
